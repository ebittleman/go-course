...
const PRESENTPATH = "/present/"

func init() {
	http.HandleFunc(PRESENTPATH, dirHandler)
}
...
func dirHandler(w http.ResponseWriter, r *http.Request) {
	...
	const base = "."
	path := r.URL.Path
	name := filepath.Join(base, path[len(PRESENTPATH):])
	if isDoc(name) {
	...
	http.FileServer(http.Dir(name)).ServeHTTP(w, r)
}
...
func dirList(w io.Writer, name string) (isDir bool, err error) {
	...
	for _, fi := range fis {
		...
		e := dirEntry{
			Name: fi.Name(),
			Path: filepath.ToSlash(filepath.Join(PRESENTPATH[1:], name, fi.Name())),
		}
		...
		if isDoc(e.Name) {
			...
			// e.Path = filepath.ToSlash(filepath.Join(PRESENTPATH[1:], e.Path))
